...
sub header_letter_box {
	...
	if (-f "$userdir/$id/letter_flag.cgi") {
		open my $fh, "< $userdir/$id/letter_flag.cgi" or &error('ﾚﾀｰﾌｧｲﾙの読み込みに失敗しました');
		my $line = <$fh>;
		close $fh;
		@letters = split /<>/, $line;
	}
	...
	if ($is_rewrite && !$is_delete) { # letter_flag.cgi が削除されないなら未読状態を更新
		open my $fh, "> $userdir/$id/letter_flag.cgi" or &error('ﾚﾀｰﾌｧｲﾙの書き込みに失敗しました');
		my $line = '';
		...
		unlink "$userdir/$id/letter_flag.cgi" if -f "$userdir/$id/letter_flag.cgi";
	}
