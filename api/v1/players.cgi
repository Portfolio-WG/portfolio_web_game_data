	...
	if ($method eq 'GET') {
		...
		if ($lib[1] ne '') {
			if ($lib[1] eq 'all') {
				...
				$lib[1] = 0 if $lib[1] >= $hash_data{'countries'}{'length'};
				$country = $lib[1];
...
sub get_players {
	my $country = shift;
	my $count = 0;
	open my $fh, "< $root/log/$country/member.cgi" or &error("member.cgi ﾌｧｲﾙが開けません");
	while (my $line = <$fh>) {
		$line = decode("Shift_JIS",$line);
		$line =~ tr/\x0D\x0A//d;
		$hash_data{'players'}{'name'}[$count] = $line;
		$hash_data{'players'}{'id'}[$count] = unpack 'H*',$line;
		$count++;
	}
	$hash_data{'players'}{'length'} = $count;
	close $fh;
}
