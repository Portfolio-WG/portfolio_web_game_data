...
@skills = (
	...
	[3,		'\•¶ša‚è',	'Œ•',	15,		sub{ 					&_d( $_[0]*rand(1)+0.7 );	},	'',sub{}, 20],
	[4,		'”na‚è',		'Œ•',	21,		sub{ $risk=$_[0]*0.1;	&_d( $_[0]*1.8 );			},	'',sub{}, 20],
	[5,		'”g“®Œ•',		'Œ•',	28,		sub{ $is_direct=1;		&_d( rand($_[0]) );			},	'',sub{}, 20],
	[6,		'¾•—Œ•',		'Œ•',	16,		sub{ $is_direct=1;		&_d( rand(60)+80 );			},	'•KE!¾•—Œ•!',sub{}, 20],
	[7,		'ŒŒõŒ•',		'Œ•',	10,		sub{ my $v=int(rand(50)+50); &_d( $v ); &_heal( $v );},	'Œ‚ÌŒõ‚æ‰ä‚É—Í‚ğ!ŒŒõŒ•!',sub{}, 10],
	[8,		'—´”ö•Ô‚µ',		'Œ•',	50,		sub{ $is_guard_s=1;									},	'',sub{ ${$who.'_is_guard'} = 1; }, 5],
 	[9,		'—‚êáŒ‰Ô',	'Œ•',	30,		sub{ 					&_d( 1.8, 'ag');			},	'•‘U‚ê!—‚êáŒ‰Ô!',sub{}, 20],
	...
	[19,	'ŠˆEb_Õ',	'‘„',	99,		sub{ $is_direct=1;		&_d( $_[0]*(rand(0.5)+0.5) );},	'uE!ŠˆEb_Õ!',sub{}, 30],
	[20,	'–³‘oO’i',		'‘„',	50,		sub{ $risk=$_[0]*0.1;	&_d( $_[0]*2.2 );			},	'”é“`!–³‘oO’i!',sub{}, 20],
	...
	[28,	'ÃŞ¯ÄŞØ°½Ëßİ',	'•€',	55,		sub{ $risk=$_[0]*0.2;&_d( $_[0]*2.5 );				},	'•KE!ÃŞ¯ÄŞØ°½Ëßİ!',sub{ ${$who.'_is_stanch'} = 1; }, 20],
	[29,	'–‚la‚è',		'•€',	99,		sub{ 					&_d( $_[0]*int(rand(2))*3 );},	'‹°•|‚Æ‚¢‚¤–¼‚ğ‚»‚Ì‹¹‚É‚ß!–‚la‚è!',sub{}, 30],
	...
	[35,	'–‚‰¤‚Ì‰Š',		'‰Š',	30,		sub{ $is_direct=1;	&_d( rand(0.5)+0.5, 'mat' );	},	'‘å’n‚Éõ‚İ“n‚é •œQ‚ÌÔ‚¢ŒŒ‚æ ‚»‚Ìg–½‚ğ‰Ê‚½‚¹I',sub{}, 20],
	[36,	'»İÌÚ²Ñ',		'‰Š',	38,		sub{ $risk=$_[0]*0.35;	&_d( $_[0]*2.5 );			},	'',sub{}, 20],
	[37,	'‰ŠE—ó”g',		'‰Š',	40,		sub{ $is_direct=1;&_d( rand(70)+160 );				},	'',sub{ ${$who.'_is_stanch'} = 1; }, 20],
	[38,	'’n–‚Ì‹Æ‰Î',	'‰Š',	45,		sub{ $risk=$_[0]*0.2;$is_direct=1;&_d( $_[0]*0.7 );	},	'’n–‚©‚ç¢Š«‚³‚ê‚µg˜@‚Ì‰Š!',sub{}, 20],
	[39,	'Ì¨Å°Ú',		'‰Š',	70,		sub{ $risk=$_[0]*0.2;$is_direct=1;&_d( rand($_[0])*3 );},	'Ià‚Ì¢ŠE‚Öc“™‚µ‚­–Å‚Ñ‚ğ—^‚¦‚ñ‚±‚Æ‚ğ!',sub{}, 30],
	[40,	'µ°ÊŞ°ÄŞ°½Ş',	'‰Š',	99,		sub{ $risk=$_[0]*0.3;$is_direct=1;&_d( rand(3)+1, 'mp' );},	'k‚¦‚ëA–½‚Â‚È‚¬~‚ß‚éŒõA¡‘S‚Ä‚ğ‰ğ‚«•ú‚ÄIµ°ÊŞ°ÄŞ°½Ş!',sub{}, 30],
	...
	[49,	'“Vg‚Ì‰H',		'•—',	40,		sub{ $is_direct=1;&_d( rand($_[0]) );&_heal( rand($_[0]*0.3) );},'¡“ğ‚Ì–¼‚É‚¨‚¢‚ÄAâq‚ê‚µÒ‚ğò‰»‚¹‚æ!',sub{}, 10],
	...
	[52,	'—‹‚Ìô”›',		'—‹',	8,		sub{ &_mp_d( rand($_[0]*0.3) );						},	'',sub{ ${$who.'_is_stanch'} = 1; }, 14],
	[53,	'Ï¼Ş¯¸½Ã¨°Ù',	'—‹',	90,		sub{ $is_guard_s=1; my $v=rand(30); &_mp_d( $v ); &_mp_heal( $v*0.8 );	},	'',sub{ ${$who.'_is_guard'} = 1; }, -5],
	[54,	'ˆîÈa‚è',		'—‹',	10,		sub{ 					&_d( $_[0]*1.3 );			},	'',sub{}, 15],
	[55,	'ºŞ¯ÄŞÊİÄŞ',	'—‹',	15,		sub{ &_d( 1.5, 'mat' );								},	'–‚‚Ì—‚ğ”w•‰‚¢AŒõ‚Æ‚È‚è‚ÄW‚¦!',sub{ ${$who.'_is_stanch'} = 1; }, 20],
	...
	[59,	'Œ‚ÌÙ‚«',		'—‹',	30,		sub{ $is_direct=1;		&_d( rand(150)+80 );		},	'‚±‚ÌÒ‚ğ”±‚¹‚æ!Œ‚ÌÙ‚«!',sub{}, 20],
	[60,	'—´—‹_',		'—‹',	60,		sub{ &_d( $_[0]*2.0 );								},	'“V‚æ‚è•‘‚¢~‚è‚µ—‹‚Ì—´‚æ!',sub{ ${$who.'_is_stanch'} = 1; }, 30],

	[61,	'¸Ş×İÄŞ¸Û½',	'–³',	20,		sub{ &_d( 1.4, 'df');								},	'•KE!¸Ş×İÄŞ¸Û½!',sub{ ${$who.'_is_counter'} = 1; }, 20],
	[62,	'ÊßÌÊßÌ',		'–³',	15,		sub{ my $v=rand(0.3); &_mp_d( $v,'cha' ); &_mp_heal( $v*0.5,'cha' );	},	'™Ê§Ä™',sub{}, -5],
	[63,	'”š—óŒ',		'–³',	30,		sub{ 					&_d( rand($_[0]*3) );		},	'‚­‚ç‚¦!”šE—óEŒ!',sub{}, 20],
	...
	[69,	'—¬¯R‚è',		'–³',	54,		sub{ 					&_d( rand(1)+1.5, 'ag' );	},	'•KE!—¬¯R‚è!',sub{}, 30],
	[70,	'—´_—ó‰ÎŒ',	'–³',	99,		sub{ $is_direct=1;		&_d( $_[0]*(rand(1)+0.2) ); },	'g‚Ì‚‚È‚é‚ÍS‚Ì‚‚Æ‚È‚ç‚´‚é‚È‚è!–û’f‘å“G!—´_—ó‰ÎŒ!',sub{}, 30],
);

# ------------------
sub _d {#ˆø‚«Z‚Å‚Í‚È‚­UŒ‚/–hŒäA“G‚©‚ç‚Ì”íƒ_ƒ[ƒW”÷¬
	my($v, $k) = @_;

	if ($who eq 'm') {
		$v = 0.5 * ($v + 1) * $m{$k} if defined $k;
#		$v = $v * 0.05 - $y_df * 0.05 unless $is_direct;
		$v = int($v / $y_df * 200) unless $is_direct;
		$v = int($v * 0.3) if $is_direct;
		$v = $v < 0 ? int( rand(5) ) : int( $v * (rand(0.3)+ 0.9) );
		$y{hp} -= $v;
		$mes .= "$y{name} ‚É $v ‚ÌÀŞÒ°¼Ş‚ğ‚ ‚½‚¦‚½!<br>";
	}
	elsif ($who eq 'y') {
		$v = 0.5 * ($v + 1) * $y{$k} if defined $k;
#		$v = $v * 0.05 - $y_df * 0.05 unless $is_direct;
		$v = int($v / $m_df * 80) unless $is_direct;
		$v = int($v * 0.2) if $is_direct;
...
sub _risk {
	...
	if ($who eq 'm' && !($guas[$m{gua}][0] eq '18' && rand(10) < 3)) {
		$v = $v < 0 ? int( rand(5)+1 ) : int( $v * (rand(0.3)+ 0.9) * 0.4);
		$m{hp} -= $v;
		$mes .= "$m{name} ‚Í ‹Z‚Ì”½“®‚Å $v ‚ÌÀŞÒ°¼Ş‚ğ‚¤‚¯‚½!<br>";
	}
	elsif ($who eq 'y' && !($guas[$y{gua}][0] eq '18' && rand(10) < 3)) {
		$v = $v < 0 ? int( rand(5)+1 ) : int( $v * (rand(0.3)+ 0.9) * 0.6);
		$y{hp} -= $v;
