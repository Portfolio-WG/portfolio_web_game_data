...
sub tp_120 { # flock
	...
	while (my $line = <$fh>) {
		...
		if ($cmd eq $no) {
			...
			# ‘å‚µ‚½‚±‚Æ‚¶‚á‚È‚¢‚¯‚ÇA‚·‚Å‚É“o˜^‚³‚ê‚Ä‚¢‚é”ñ‘¶İ‚ÌPC‚Æ“¯‚¶–¼‘O‚É‰ü–¼‚·‚é‚Æ“o˜^—¿“¥‚İ“|‚¹‚»‚¤
			# ‚»‚ê‚æ‚è‚à”ñ‘¶İ‚Ì·¬×‚ğ•\¦‚³‚ê‚Ä‚à×–‚‚¾‚©‚ç“o˜^‚É”rœ tp_210
			push @lines, $line;
		}
	}
...
sub tp_210 { # flock
	...
	while (my $line = <$fh>) {
		...
		push @lines, $line;# if &you_exists($name); # ”ñ‘¶İ‚Ì·¬×‚ÍÁ‹‚µ‚½‚¢‚ªAƒ^ƒCƒ~ƒ“ƒO‚É‚æ‚Á‚Ä‚ÍÃŞ°À‚Ì”ÍˆÍŠO‚ÉƒAƒNƒZƒX‚³‚ê‚é‚½‚ß‚ÉƒŠƒXƒg–„‚Ü‚é‚æ‚¤‚É‚µ‚Ä‚¢‚éH
...
sub tp_310 { # flock
	...
	if ($cmd) {
		...
		if ($is_marriage) {
			...
			&regist_you_array($name, @you_data); # Ì§²Ùµ°Ìßİ”3‰ñ¨1‰ñ‚Éˆ³k
			undef @you_data;
			...
			# “o˜^‚³‚ê‚Ä‚¢‚é–¼‘O‚ğíœ
			&delete_entry_marriage($m{name}, $name); # Ì§²Ùµ°Ìßİ”4‰ñ¨Å‚2‰ñ‚Éˆ³k
		}
	}
...
#	my $del_name = shift || $m{name};
	...
	# Ì§²Ùµ°Ìßİ”íŒ¸
	# –³‰ü‘¢‚¾‚Æ’j——¼•û‚ÌÌ§²Ù‚ğ‘€ì‚·‚é‚½‚ß•K‚¸2‰ñŠJ‚­•K—v‚ª‚ ‚é‚ªAÕØÊŞ×‚Í1‰ñ‚ÉÏ‚Ş‚Ì‚ÅÈ—ª
	# ‚Ü‚½A«“]Š·‚µ‚½ê‡‚È‚Ç‚ğ‘z’è‚·‚é‚Ææ‚è‚±‚Ú‚·‰Â”\«‚ª‚ ‚é‚ª•“ØI‚©‚ç‚»‚¤‚¾‚µ•Ê‚Ìˆ—‚Å”rœ‚³‚ê‚é‚½‚ß–³‹
	my @files;
