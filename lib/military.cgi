...
sub tp_710 {
	...
	if (-s "$userdir/$id/ambush.cgi") {
		...
		$m{turn} = @lines;
		$m{ambush_on} = 1;#iŒR‘Ò‚¿•š‚¹‚ÌŒoŒ±’l‚ğ•ªŠò‚³‚¹‚é‚½‚ß ‘¼‚ÌŒoŒ±’l‚Ì•ªŠò‚É‚àg‚¦‚é‚©‚àH by‚ ‚¨‚Ì‚è
	}
...
sub exe3 {
	...
	$m{stock} = &use_pet($l, $m{stock}) unless (($w{world} eq '17' || ($w{world} eq '19' && $w{world_sub} eq '17')) && ($m{pet} ne '33' && $m{pet} ne '34' && $m{pet} ne '35'));
...
sub tp_1100 {
	...
	else{#iŒR‘Ò‚¿•š‚¹‚ÌŒoŒ±’l”X•ªŠò—p í‘ˆŸ—˜‚Ì30“
		my $v = int( (rand(9)+9) * $m{turn} );
		$v = &use_pet('military', $v) unless (($w{world} eq '17' || ($w{world} eq '19' && $w{world_sub} eq '17')) && $m{pet} ne '161');
		$m{exp} += $v;
		$mes .= "$v‚Ì$e2j{exp}‚ğè‚É“ü‚ê‚Ü‚µ‚½<br>";
		$m{medal} += int(0.4 * $m{turn});
		$m{egg_c} += int(rand($m{turn})+$m{turn}) if $m{egg};
#		$m{egg_c} += int(rand($m{turn})+$m{turn}) if $m{egg};
#		$m{ambush_on} = 0;
	}
	if ($m{turn} >= 5 && $m{ambush_on} == 0) {
		$mes .= "”C–±‚É‘å¬Œ÷!$m{name}‚É‘Î‚·‚é•]‰¿‚ª‘å‚«‚­ã‚ª‚è‚Ü‚µ‚½<br>";
		$m{rank_exp} += $m{turn} * 3;
	}
	elsif($m{ambush_on} == 1){
		$mes .= "”C–±‚É¬Œ÷!$m{name}‚É‘Î‚·‚é•]‰¿‚ªã‚ª‚è‚Ü‚µ‚½<br>";
		$m{rank_exp} += int($m{turn} * (rand(4) + 6));
	}
	else {
		$mes .= "”C–±‚É¬Œ÷!$m{name}‚É‘Î‚·‚é•]‰¿‚ªã‚ª‚è‚Ü‚µ‚½<br>";
		$m{rank_exp} += int($m{turn} * 1.5);
	}
	$m{ambush_on} = 0;#”O‚Ì‚½‚ß‚±‚±‚É’u‚¢‚Ä‚¨‚­
