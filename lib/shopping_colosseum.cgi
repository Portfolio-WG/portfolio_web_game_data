...
sub _defence_c_up {
	...
	while (my $line = shift @temp_lines) {
		if ($count == $count_sub - 3 + $m{value}) {
			my($name,$country,$max_hp,$max_mp,$at,$df,$mat,$mdf,$ag,$cha,$wea,$skills,$mes_win,$mes_lose,$icon,$defence_c) = map { $_ =~ tr/\x0D\x0A//d; $_; } split /<>/, $line;
			++$defence_c;
	...
	for my $defence_line (@defence_lines) {
		my($name,$country,$max_hp,$max_mp,$at,$df,$mat,$mdf,$ag,$cha,$wea,$skills,$mes_win,$mes_lose,$icon,$defence_c) = map { $_ =~ tr/\x0D\x0A//d; $_; } split /<>/, $defence_line;
		++$defence_c;
