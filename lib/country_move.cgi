...
sub tp_110 {
	...
	while (my $line = <$fh>) {
		...
		if($i_c == $cmd){
			my($hname, $hcountry) = split /<>/, $line;
			if ($hcountry eq $m{country}) { # 自国からの勧誘はﾍﾄﾞﾊﾝ効果消失
				$mes .= "自国に仕官はできません<br>";
				&begin; # 確認してないけど begin で良いのか微妙
			}
			elsif (defined $cs{name}[$hcountry]) { # 国が存在する
				if ($m{country}) {
					if ($m{name} eq $cs{ceo}[$m{country}]) { # 君主ならﾍﾄﾞﾊﾝ効果持続
	...
	if (defined($from_name) && defined($from_country)) {
		...
		$m{vote} = '';
		&mes_and_world_news("$from_nameの誘いで$cs{name}[$from_country]に仕官しました",1);
	}
