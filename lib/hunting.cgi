...
# 超ボスもしくが出てくる確率(分の1)
my $boss_par = $pets[$m{pet}][2] eq 'hunt_lv' ? 500:
				$pets[$m{pet}][2] eq 'no_boss' ? 10000:
				20000;
...
#冬イベ　ステータスUP確率
my $get_event_status_par = 25;
#冬イベ　ステータスUP量
my $winter_status = 15;
#冬イベ　隠しステージ確率
my $secret_stage_par = 2000;#1/2000
#冬イベ用のステージ数
my $winter_stage = 2;
#=================================================
# 利用条件
...
#		if(!&on_new_year || $m{winter_secret_stage} eq 0){
#			$m{stock}++;
#		}
		&_get_hunt_you_data;
	}
...
sub _get_hunt_you_data {
	...
	if (-f "$datadir/metal.cgi" && (!$m{no_boss} || $data_num eq 'boss') && 5 < $m{stock} && rand($metal_par) < 1 && $data_num ne 'event') { # ﾎﾞｰﾅｽﾓﾝｽﾀｰ
	...
	elsif (rand($npc_par) < 1 || $data_num eq 'event') { # NPCﾓﾝｽﾀｰ
		if(&on_new_year){#冬イベ用
			require "$datadir/npc_winter_event_hunting.cgi";
		}
		else{
...
#			$m{egg_c} += int(rand($m{stock}-1)) if $m{egg};
		...
		if(int(rand($get_event_status_par)) == 0 && $places[$m{stock}][0] eq 'event'){
			&status_up_winter_event;
		}
		if(int(rand($secret_stage_par)) == 0 && $places[$m{stock}][0] eq 'event'){
			&secret_stage_appear;
		}
...
#=================================================
# 冬イベ用、追加ステータスをランダムに選びUP
#=================================================
sub status_up_winter_event{
	my $status_kind = rand(5);
	@keys_status = (at_e,df_e,mdf_e,mat_e,ag_e);
	@keys_status_name = ("攻撃","防御","魔防","魔力","素早さ");
	$m{$keys_status[$status_kind]} += $winter_status;
	$mes .="$keys_status_name[$status_kind]が$winter_status上がった！<br>";
}
#=================================================
# 冬イベ用、隠しステージ
#=================================================
sub secret_stage_appear{
	$m{winter_secret_stage} = 1;
	$mes .= qq|<font color="#FF00FF">エイボンの書を手に入れた</font><br>|;
	&write_world_news("$m{name}はエイボンの書を手に入れた", 1);
}
#=================================================
# 適当なアイコンを表示
#=================================================
