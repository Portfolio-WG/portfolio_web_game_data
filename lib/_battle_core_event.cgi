require "$datadir/skill_event.cgi";
...
if(&on_december_end || (&on_new_year && !on_new_year_end)){#“~ƒCƒx—p‚ÌƒXƒe[ƒ^ƒX’êã‚°ˆ—
	$m_at += 500 + $m{at_e};
	$m_df += 500 + $m{df_e};
	$m_mdf+= 500 + $m{mdf_e};
#	$m_mat = 500 + $m{mat_e} + $m{mat};
	$m_ag += 500 + $m{ag_e};
}#í“¬‰æ–Ê‚©‚ç–ß‚é‚Æ‚«‚Éhp‚Ìó‘Ô‚ğ•Û‚·‚é•K—v‚ª‚ ‚é
...
if    ($weas[$m{wea}][2] =~ /–³|Œ•|•€|‘„/) { $m_at = $m_at  + $weas[$m{wea}][3]; }
elsif ($weas[$m{wea}][2] =~ /‰Š|•—|—‹/)    { $m_at = 500 + $m{mat_e} + $m{mat} + $weas[$m{wea}][3]; $y_df = $y_mdf; }
...
sub attack {
	...
	if (defined($skill)) { # •KE‹Z
		...
		for my $scc (1..$sc) {
			...
			if (${$temp_y.'_is_counter'} && rand(2) == 0) {
				$mes .= "UŒ‚‚ğ•Ô‚³‚ê $v ‚ÌÀŞÒ°¼Ş‚ğ‚¤‚¯‚Ü‚µ‚½<br>";
				${$who}{hp} -= $v;
			}
		elsif (${$temp_y.'_is_stanch'} && rand(2) == 0) {
				$mes .= "½Àİ‚Å“®‚¯‚È‚¢!<br>";
			}
			else {
				$mes .= "$v ‚ÌÀŞÒ°¼Ş‚ğ";
...
#	my $v = int( ($at * 0.5 - $df * 0.3) * (rand(0.3)+ 0.9) );
	my $v = int( (0.5 * $at / $y_df * 200) * (rand(0.3)+ 0.9) );
