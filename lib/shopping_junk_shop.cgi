...
sub tp_300 {
	...
	--$cmd;
	my $time_gacha_on = $time - 3600 * 4;
	my $gacha_time_on = $m{gacha_time} - 3600 * 4;
	if (&time_to_date($time_gacha_on) eq &time_to_date($gacha_time_on)) {
		my $g_time = $m{gacha_time} - $time + 3600 * 24;
...
sub tp_400 {
	...
	--$cmd;
	my $time_gacha_on = $time - 3600 * 4;
	my $gacha_time2_on = $m{gacha_time2} - 3600 * 4;
	if (&time_to_date($time_gacha_on) eq &time_to_date($gacha_time2_on)) {
		my $g_time2 = $m{gacha_time2} - $time + 3600 * 24;
