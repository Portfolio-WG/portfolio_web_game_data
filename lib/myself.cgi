...
if ($m{valid_blacklist}) {
	...
	push @menus, ['Œğ—¬ÌŞ×Ø', 'myself_blacklist_chatpublic'];
	push @menus, ['‰ï‹cºÌŞ×Ø', 'myself_blacklist_country_bbs'];
}
...
sub my_status_pc {
	...
	if ($m{pet}) {
		...
		if (&on_december_end || (&on_new_year && !&on_new_year_end)) {
			$mes .= qq|y²ÍŞİÄ½Ã°À½z<br>|;
			$mes .= qq|<table class="table1" cellpadding="3">|;
			$mes .= qq|<tr><th>UŒ‚</th><td>$m{at_e}</td>|;
			$mes .= qq|<th>–hŒä</th><td>$m{df_e}</td>|;
			$mes .= qq|<th>–‚—Í</th><td>$m{mat_e}</td>|;
