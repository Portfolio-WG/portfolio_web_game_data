...
#一度に購入できるサマージャンボの枚数上限
my $lot_ticket_limit = 200;
	...
	if ($cmd eq '1') {
		...
		if (-f "$this_lot_name_file") {
			...
			if ($lot_amount) {
				...
				$mes .= '計' . $lot_amount . '枚買っているよ<br>';
			}
		}
		$mes .= "$lot_ticket_limit 枚まで一度に買えるよ<br>";
		$mes .= qq|<form method="$method" action="$script">|;
		$mes .= qq|<input type="text" name="lot_ticket" value="1" class="text_box1" style="text-align:right">枚|;
					...
					if ($par >= $r_par) {
						...
						if ($m{money} >= $money) {
							...
							$mes .= $item[0] eq '1' ? "$weas[$item[1]][1]"
								  ...
								  ;
							$mes .= 'を買ったよ!';
							&send_item($m{name}, $item[0],$item[1],$item[2],$item[3]);
...
sub tp_300 {
	...
	if($in{lot_ticket} && $in{lot_ticket} < 1){
		$mes.= "有効な数字を入力してね<br>";
		&begin;
		return;
	}elsif($in{lot_ticket} && $in{lot_ticket} > $lot_ticket_limit){
		$mes.= "1度に買う枚数が多すぎるみたい…<br>";
		&begin;
		return;
	}elsif($in{lot_ticket} && $m{money} < $lot_money * $in{lot_ticket}){#突貫工事感ある　あとで直したい
		$mes .= "お金が足りない…<br>";
		&begin;
		return;
	}
	...
	if ($m{money} >= $lot_money) {
		...
		my $pre_max_lot = $max_lot;
		if($in{lot_ticket}){
			$max_lot += $in{lot_ticket};
		}else{
			$max_lot++;
		}
		push @lines, "$max_lot<>\n";
		...
		if($in{lot_ticket}){
			...
			for my $hi (1..$in{lot_ticket}) {
				my $lot_num = sprintf("%06d", $pre_max_lot + $hi);
				print $fhn "$m{name}<>$lot_num<>\n";
			}
			close $fhn;
			$m{money} -= $lot_money * $in{lot_ticket};
			$mes .= "$in{lot_ticket} 枚買いました<br>";
		}else{
...
sub tp_810 {
	...
	for my $hi (0..$#morning_glory_height) {
		if ($m{morning_glory} >= $morning_glory_height[$hi][0]) {
			$mes .= $morning_glory_height[$hi][1] . '<br>';
			last;
		}
	}
	if ($cmd) {
...
sub tp_910 {
	...
	if($in{vote_name} eq $admin_sub_name){
		$mes .= '気持ちだけ頂くよ、応援ありがとう！<br>';
		&begin;
		return;
	}
