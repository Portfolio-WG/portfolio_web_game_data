...
#================================================
# 仮装(ﾊﾛｳｨﾝ限定、0G、整形に埋め込みたかったけど改造点が多いし複雑化したくなかったため分離)
#================================================
sub tp_700 {
	if ($default_icon eq '') {
		$mes .= 'ごめんなさぁい。この病院には仮装のﾄﾞｸﾀｰがいないのぉ<br>';
		&begin;
		return;
	}
	if ($m{icon_t} ne '') {
	...
	$mes .= "$menus[7][0]は、貴方を全くの別人に変装させちゃうわよぉ<br>";
	$mes .= "ﾊﾛｳｨﾝだからﾀﾀﾞで施術するけどぉ<br>";
	$mes .= "どぉするぅ？<br>";
	$m{tp} += 10;
	&menu("やめる", "自分のｱｲｺﾝに$menus[7][0]する", "ﾃﾞﾌｫﾙﾄｱｲｺﾝに$menus[7][0]する");
}
sub tp_710 {
	return if &is_ng_cmd(1..2);
	if ($default_icon eq '') {
	...
	$layout = 2;
	$mes .= 'どのような仮面にしますぅ?<br>ｶﾀﾛｸﾞからお選びくださぁい<br>';
	unless ($m{icon} eq $default_icon) {
		my $file_title = &get_goods_title($m{icon});
		$mes .= "現在の顔ｱｲｺﾝﾀｲﾄﾙ『$file_title』<br>";
	}
...
sub tp_720 {
	...
	if ($in{icon} && ($in{icon} eq $default_icon || -f "$icondir/$in{icon}" || -f "$userdir/$id/picture/$in{icon}") ) {
		if ($m{money} >= $menus[4][1]) {
			...
			$mes .= '麻酔を打って手術を始めるわね<br>';
			$mes .= '次に目覚めたときにわぁ別人となっているわよぉ<br>';
			$mes .= '喋り方が変わっちゃうかもしれないけどすぐ治っちゃうから安心してねぇ<br>';

			my $silent_halloween = 'ﾊﾛｳｨﾝ!';
			if($m{shogo} ne $shogos[1][0] && $m{shogo_t} ne $shogos[1][0]){
				$m{shogo} = "★ﾊﾛｳｨｰﾝ";
#				$m{shogo} = "★テスト";
			}
			if(rand(4) < 1){
				my $silent_rand = int(rand(4) + 1);
				$m{silent_time} = $time + 1200;
				$m{silent_kind} = $silent_rand;
				$m{silent_tail} = $silent_halloween;
			}
