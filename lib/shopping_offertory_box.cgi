...
#乱数なので大きめに設定すること
my $flow_total = 30000000;
#以下2つは計算用　一次関数で100万(50分の1)を基準として調整
my $flow_max = 11000000;#総額がこの値以上で当たった時、累計額が0になる
my $flow_grad = 10;#一次関数の傾き
...
sub tp_100 {
	...
	if ($cmd eq '1' && $in{send_money} > 0 && $in{send_money} !~ /[^0-9]/) {
		if ($m{money} >= $in{send_money}) {
			...
			#新春おみくじ処理 2021_12_30
			#バグが多かったので、緊急処理としてomikujiからomikuji2にしてもう一度回せる様にした　来年までに必ず変更する
			#omikuji,omikuji2の削除処理をlotに作っておくこと
			if(&on_new_year && !-f "$userdir/$id/omikuji2.cgi"){
				$mes .= &mes_omikuji;
				open my $fh, "> $userdir/$id/omikuji2.cgi" or &error("$userdir/$id/omikuji2.cgi ﾌｧｲﾙが作れません");
				close $fh;
			}

#			$total += $in{send_money};#ｻﾏｼﾞｬﾝ対策で変更
			if($in{send_money} >= 5000000){#上限額
				$total += 5000000;
			}else{
				$total += $in{send_money};
			}
...
#				$total -= 2000000;#ｻﾏｼﾞｬﾝ対策で変更
				$total = int(($flow_max - $total) / $flow_grad);
				$total = 0 if $total < 0;
				if (rand(10) < 1) {
					&get_god_item(7);
...
#おみくじのメッセージ,吉凶作成
#神様降臨処理
#必要データは全て$datadir/omikuji.cgiに格納
sub mes_omikuji {
	require "$datadir/omikuji.cgi";
	my $rand_omikuji = int(rand(200) + 1);
	my $m_luck = 0;
	my $tmp_mes .= "";
	my $letter_mes .= "";
	for my $i(0..$#luck_omikuji){
		if($rand_omikuji >= $luck_omikuji[$i][2]){
			$m_luck = $i;
		}
	}
	...
	$letter_mes .= '<br>#=======================#<br>';
	$letter_mes .= '運勢:';
	$letter_mes .= "【$luck_omikuji[$m_luck][1]】<br>";
	$letter_mes .= '#=======================#<br>';

	for my $omikuji_kind(@omikuji_kinds){#各ｺﾏﾝﾄﾞの運勢に対し
		my $rand_omikuji_kind = int(rand(22) + 1);#1-22の22段階で各運勢を評価
		my $count = 0;
		my @test = @{$rand_lucks[$m_luck]};
		for my $i(0..$#test){#8段階の各全体運勢に対し、6通りの運勢
			$count += $rand_lucks[$m_luck][$i];
			if($count >= $rand_omikuji_kind){#もし22段階rand数値が各6通り運勢の基準値を上回っていれば
				#ここ処理が重いので心配,,
				my $m_rand = int(rand($#{$mes_lucks[$i]}) + 1);
				$letter_mes .= "【$omikuji_kind】 $mes_lucks[$i][0],$mes_lucks[$i][$m_rand]<br>";
				last;
			}
		}
	}
	...
	$in{comment} .= $letter_mes;
	&send_letter($m{name},1);#手紙を送る

	if($m_luck eq $#luck_omikuji && !-f "$userdir/$id/omikuji.cgi"){#大大吉のとき
		&get_god_item(7);

		$tmp_mes .= "黒豚神<ぶー<br>";
		&mes_and_world_news("黒豚神様から新年のアイテムをもらいました", 1);
	}
	return $tmp_mes;
}
