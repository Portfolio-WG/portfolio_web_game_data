...
sub tp_210 {
	if ($in{file_name}) {
		...
		if($goods_dir eq "book" ){#お題コンテスト用に同期してあるetcファイルの操作　etcファイルがいっぱいになるのを懸念して捨てる送るだけ操作追加　by あおのり
			unlink "$userdir/$id/etc/$in{file_name}" or &error("選択した$goods_name($file_title)が存在しません");
		}
...
sub tp_310 {
	...
	if ($m{money} < $pay) {
		...
		if($goods_dir eq "book" ){#お題コンテスト用に同期してあるetcファイルの操作　by あおのり
			rename "$userdir/$id/etc/$in{file_name}", "$userdir/$send_id/etc/$in{file_name}" or &error("$goods_nameを送るのに失敗しました");
		}
		my $file_title = &get_goods_title($in{file_name});
